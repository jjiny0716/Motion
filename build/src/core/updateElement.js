export function updateElement(parent, newNode, oldNode) {
    ...
    if (oldNode.hasAttribute("data-component"))
        return convertNodeToComponentData(oldNode);
    let childComponentData = {};
    const newChildren = [...newNode.childNodes];
    const oldChildren = [...oldNode.childNodes];
    const maxLength = Math.max(newChildren.length, oldChildren.length);
    for (let i = 0; i < maxLength; i++) {
        childComponentData = Object.assign(Object.assign({}, childComponentData), updateElement(oldNode, newChildren[i], oldChildren[i]));
    }
...
function convertNodeToComponentData(node) {
    ...
    for (let child of node.childNodes) {
        childComponentData = Object.assign(Object.assign({}, childComponentData), convertNodeToComponentData(child));
    }
    return childComponentData;
}
